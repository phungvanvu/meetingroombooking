...
public class StatisticalServiceImpl implements StatisticalService {
  ...
  public RoomStatisticsDTO getRoomStatistics() {
    ...
    List<Room> unavailableRooms = allRooms.stream().filter(r -> !r.isAvailable()).toList();
    List<RoomBooking> allBookings = roomBookingRepository.findAll();
    long totalBookings = allBookings.size();
    LocalDate today = LocalDate.now();
    List<RoomBooking> todayBookings =
        allBookings.stream().filter(b -> b.getStartTime().toLocalDate().equals(today)).toList();
    List<RoomDTO> availableRoomDTOs = availableRooms.stream().map(roomMapper::toDTO).toList();
    List<RoomDTO> unavailableRoomDTOs = unavailableRooms.stream().map(roomMapper::toDTO).toList();
    List<RoomBookingDTO> todayBookingDTOs =
        todayBookings.stream().map(roomBookingMapper::toDTO).toList();
  ...
  public List<BookingSummaryDTO> getWeeklyBookings() {
    ...
    return results.stream()
        .map(
            arr ->
                new BookingSummaryDTO(((Number) arr[0]).intValue(), ((Number) arr[1]).intValue()))
